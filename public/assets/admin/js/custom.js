$(function ($) {
    ...
    $(document).on('change', '.up-img', function () {
        ...
        var timeNowInSeconds = new Date().getTime();
        var valueForDB = 'module_text_with_image_' + timeNowInSeconds + '_' + filename;
        $(this).siblings('.file-image-value').val(valueForDB);
...
$('input.fix_slug').on('change', function () {
    if($(this).is(":checked")) {
        $(".sluginp").attr('readonly', 'readonly');
    } else {
        $(".sluginp").removeAttr('readonly');
    }
});