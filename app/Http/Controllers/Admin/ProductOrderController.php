...
class ProductOrderController extends Controller
{
    ...
    {
        $data['orders'] = Order::when($request['order_number'], function ($q) use ($request) {
                                return $q->where('order_number', 'LIKE', '%' . $request['order_number'] . '%');
                            })->when($request['total'], function ($q) use ($request) {
                                return $q->where('total', 'LIKE', '%' . $request['total'] . '%');
                            })->when($request['payment_status'] || $request['payment_status'] == '0', function ($q) use ($request) {
                                return $q->where('payment_status', $request['payment_status']);
                            })->orderBy('id', 'DESC')->get();

        $data['route_search']   = 'admin.all.product.orders';
        $data['status']         = 0;
    ...
    {
        $data['orders'] = Order::where('order_status', '0')->when($request['order_number'], function ($q) use ($request) {
                                return $q->where('order_number', 'LIKE', '%' . $request['order_number'] . '%');
                            })->when($request['total'], function ($q) use ($request) {
                                return $q->where('total', 'LIKE', '%' . $request['total'] . '%');
                            })->when($request['payment_status'] || $request['payment_status'] == '0', function ($q) use ($request) {
        ...
        $data['route_search']   = 'admin.pending.product.orders';
        $data['status']         = 1;
    ...
    {
        $data['orders'] = Order::where('order_status', '1')->when($request['order_number'], function ($q) use ($request) {
                                return $q->where('order_number', 'LIKE', '%' . $request['order_number'] . '%');
                            })->when($request['total'], function ($q) use ($request) {
                                return $q->where('total', 'LIKE', '%' . $request['total'] . '%');
                            })->when($request['payment_status'] || $request['payment_status'] == '0', function ($q) use ($request) {
        ...
        $data['route_search']   = 'admin.processing.product.orders';
        $data['status']         = 1;
    ...
    {
        $data['orders'] = Order::where('order_status', '2')->when($request['order_number'], function ($q) use ($request) {
                                return $q->where('order_number', 'LIKE', '%' . $request['order_number'] . '%');
                            })->when($request['total'], function ($q) use ($request) {
                                return $q->where('total', 'LIKE', '%' . $request['total'] . '%');
                            })->when($request['payment_status'] || $request['payment_status'] == '0', function ($q) use ($request) {
        ...
        $data['route_search']   = 'admin.completed.product.orders';
        $data['status']         = 1;
    ...
    {
        $data['orders'] = Order::where('order_status', '3')->when($request['order_number'], function ($q) use ($request) {
                                return $q->where('order_number', 'LIKE', '%' . $request['order_number'] . '%');
                            })->when($request['total'], function ($q) use ($request) {
                                return $q->where('total', 'LIKE', '%' . $request['total'] . '%');
                            })->when($request['payment_status'] || $request['payment_status'] == '0', function ($q) use ($request) {
        ...
        $data['route_search']   = 'admin.rejected.product.orders';
        $data['status']         = 1;
